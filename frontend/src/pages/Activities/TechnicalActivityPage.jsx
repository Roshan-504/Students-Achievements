...
import TechnicalActivityForm from '../../forms/TechnicalActivityForm';
import axiosInstance from '../../services/axiosInstance';
import toast from 'react-hot-toast';

const TechnicalActivityPage = () => {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get('/student/technical-activities');
    ...
    try {
      ...
      if (editingActivity) {
        response = await axiosInstance.put(
          `/student/update/technical-activity/${editingActivity._id}`,
          form,
        ...
        response = await axiosInstance.post(
          '/student/upload/technical-activity',
          form,
          { headers: { 'Content-Type': 'multipart/form-data' } }
        );
  ...
  const downloadCertificate = async (activity) => {
    try {
      const response = await axiosInstance.get(
        `/student/download/technical-activity/${activity._id}`,
        { responseType: 'blob' }
      );
  ...
  const deleteActivity = async (id) => {
    try {
      setLoading(true);
      await axiosInstance.delete(`student/technical-activity/${id}`);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Your Technical Activities</h1>
              <p className="text-slate-600 mt-2 max-w-2xl">
                Track your technical achievements, hackathons, and coding competitions here.
              </p>
            </div>
            <button
              onClick={() => {
        ...
        {renderActivityTable(
          ...
          "All your successfully submitted technical activities"
        )}
...
export default TechnicalActivityPage;